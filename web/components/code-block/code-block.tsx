...
import {
  type FadeEdges,
  ScrollArea,
} from "@/components/common/scroll-area/scroll-area";
...
function CodeBlock({
  ...
  const defaultProps = {
    ...
    className: cn(
      "group bg-secondary border border-border/60 max-w-full w-full rounded-xl pt-0 relative",
  ...
  const endContent = showCopy && code && (
    <div className="flex items-center gap-2">
      <CopyButton
        ...
        content={code}
        posthogEventName={copyEventName}
        posthogProperties={copyEventProperties}
      />
    </div>
  );
            ...
            <Menu.Popup className="bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 w-(--anchor-width) overflow-hidden rounded-md border p-1 shadow-md">
              {tabs.map((tab) => (
                <Menu.Item
                  ...
                  // biome-ignore lint/suspicious/noExplicitAny: Base UI internal event type mismatch
                  onClick={(event) => onTabChange?.(tab.value, event as any)}
                >
...
function CodeBlockPre({
  ...
  const defaultProps = {
    ...
    className: cn(
      "relative bg-background border border-border/60 rounded-lg whitespace-pre overflow-hidden max-h-96 flex flex-col",
